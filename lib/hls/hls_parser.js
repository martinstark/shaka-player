...
shaka.hls.HlsParser = class {
  ...
  async updateStream_(streamInfo) {
    ...
    const variablesTags = playlist.computed.playlist.variableTags;
    ...
    if (segments.length) {
      const mediaSequenceNumber = playlist.computed.playlist.mediaSequence;
      const skippedSegments = playlist.computed.playlist.skippedSegments;
    ...
    const endListTag = playlist.computed.playlist.endListTag;
  ...
  async parseManifest_(data) {
    ...
    if (playlist.type == shaka.hls.PlaylistType.MEDIA) {
      ...
      /** @type {!Array<!shaka.hls.Tag>} */
      const mediaTags = playlist.computed.playlist.mediaTags;
      /** @type {!Array<!shaka.hls.Tag>} */
      const variantTags = playlist.computed.playlist.variantTags;
      /** @type {!Array<!shaka.hls.Tag>} */
      const imageTags = playlist.computed.playlist.imageTags;
      /** @type {!Array<!shaka.hls.Tag>} */
      const iFrameTags = playlist.computed.playlist.iFrameTags;
      /** @type {!Array<!shaka.hls.Tag>} */
      const sessionKeyTags = playlist.computed.playlist.sessionKeyTags;
      /** @type {!Array<!shaka.hls.Tag>} */
      const sessionDataTags = playlist.computed.playlist.sessionDataTags;
      /** @type {!Array<!shaka.hls.Tag>} */
      const contentSteeringTags =
          playlist.computed.playlist.contentSteeringTags;
    ...
    this.manifest_ = {
      ...
      periodCount: 1,
      gapCount: 0,
      isLowLatency: playlist.computed.segments.isLowLatency,
    ...
    const downloadSegmentIndex = async (pendingRequest) => {
      ...
      try {
        ...
        const realStreamInfo = await this.convertParsedPlaylistIntoStreamInfo_(
            ...
            supplementalCodecs, mimeType, requestBasicInfo,
  ...
  async convertParsedPlaylistIntoStreamInfo_(streamId, variables, playlist,
      ...
      spatialAudio, supplementalCodecs, mimeType = undefined,
    ...
    // Accumulate gapCount from each media playlist
    if (this.manifest_) {
      this.manifest_.gapCount += playlist.computed.segments.gapCount;
    }
    ...
    const serverControlTag = playlist.computed.playlist.serverControlTag;
  ...
  makeStreamObject_(id, codecs, type, languageValue, primary, name,
      channelsCount, closedCaptions, characteristics, forced, sampleRate,
      spatialAudio, supplementalCodecs) {
  ...
  async parseDrmInfo_(playlist, mimeType, getUris, variables) {
    ...
    if (!this.config_.ignoreDrmInfo && playlist.segments) {
      for (const segment of playlist.segments) {
        const segmentKeyTags = segment.computed.keys;
  ...
  determineStartTime_(playlist) {
    ...
    const startTimeTag = playlist.computed.playlist.startTag;
    if (startTimeTag) {
      this.startTime_ =
  ...
  determinePresentationType_(playlist) {
    const PresentationType = shaka.hls.HlsParser.PresentationType_;
    const presentationTypeTag = playlist.computed.playlist.playlistTypeTag;
    const endListTag = playlist.computed.playlist.endListTag;
  ...
  determineLastTargetDuration_(playlist) {
    const lastTargetDuration = playlist.computed.segments.lastExtinfDuration ?
      playlist.computed.segments.lastExtinfDuration :
      Infinity;

    const targetDurationTag = this.getRequiredTag_(playlist.tags,
        'EXT-X-TARGETDURATION');
    const targetDuration = Number(targetDurationTag.value);
    const partialTargetDurationTag = playlist.computed.playlist.partInfTag;
  ...
  createSegmentReference_(
    ...
    const extinfTag = hlsSegment.computed.extinf;
    ...
    let syncTime = null;
    if (!this.config_.hls.ignoreManifestProgramDateTime) {
      const dateTimeTag = hlsSegment.computed.dateTime;
    ...
    // partial segments.
    const byterangeTag = hlsSegment.computed.byterange;
    if (byterangeTag) {
      [startByte, endByte] =
    ...
    if (type == shaka.util.ManifestParserUtils.ContentType.IMAGE) {
      ...
      tilesLayout = '1x1';
      const tilesTag = hlsSegment.computed.tiles;
      if (tilesTag) {
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    let discontinuitySequence =
        playlist.computed.playlist.discontinuitySequence;
    const mediaSequenceNumber = playlist.computed.playlist.mediaSequence;
    const skippedSegments = playlist.computed.playlist.skippedSegments;
    ...
    for (let i = 0; i < hlsSegments.length; i++) {
      ...
      const mapTag = item.computed.map;
      const mapTagId = item.computed.mapId;
      if (mapTag && mapTagId !== lastMapTagId) {
        currentInitSegmentRef = this.getInitSegmentReference_(playlist,
            item.tags, getUris, variables);
        lastMapTagId = mapTagId;
      }
      ...
      const discontinuityTag = item.computed.discontinuity;
      if (discontinuityTag) {
      ...
      for (const drmTag of item.computed.keys) {
      ...
      if (reference) {
        const bitrate = item.computed.bitrate;
        if (bitrate) {
          bitrates.push({
            bitrate,
