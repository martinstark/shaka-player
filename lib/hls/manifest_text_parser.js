...
shaka.hls.ManifestTextParser = class {
  ...
  parseSegments_(lines, startIndex, playlistTags, tagIndex) {
      ...
      if (line.startsWith('#EXT')) {
        ...
        if (shaka.hls.ManifestTextParser.MEDIA_PLAYLIST_TAGS.has(tag.name)) {
          ...
          if (tag.name == 'EXT-X-MAP') {
            ...
            if (tag.getAttributeValue('GAP') == 'YES') {
              gapCount++;
            }
        ...
        // Detect low-latency segments (segments with partial segments)
        if (partialSegmentTags.length > 0) {
          isLowLatency = true;
        }
    ...
    if (partialSegmentTags.length) {
      ...
      // Detect low-latency segments (final partial segments)
      isLowLatency = true;
