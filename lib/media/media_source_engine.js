...
shaka.media.MediaSourceEngine = class {
   ...
   * @return {!Promise}
   */
  async resync(contentType, timestampOffset, isDiscontinuity = false) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    // Use a tight threshold for discontinuity boundaries (where precise
    // correction matters for A/V sync) and a larger threshold for error
    // recovery (to avoid unnecessary resyncs on Safari/Tizen).
    const threshold = isDiscontinuity ? 0.001 : 0.15;
    const bufferEnd = this.bufferEnd(contentType);
    if (bufferEnd && Math.abs(bufferEnd - timestampOffset) < threshold) {
      return;
    }
